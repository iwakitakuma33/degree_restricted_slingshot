...
class PrincipalCurve:
    ...
    def project_to_curve(self, X, points=None, pseudotimes=None, stretch=0):
        ...
        for i in range(X.shape[0]):
            ...
            new_pseudotimes[i] = j + 0.1 + 0.9 * seg_proj[j]
            new_points[i] = p - proj_dist[j]

            ####
            dist_endpts = np.minimum(
                np.linalg.norm(p - points[:-1], axis=1),
                np.linalg.norm(p - points[1:], axis=1),
            )
    ...
    def fit(
        self, X, knot_size, initial_points=None, w=None, max_iter=10, tol=1e-3, ax=None
    ):
        """
        ...
        t_values = np.percentile(pseudotimes_uniq, np.linspace(1, 99, knot_size))

        tt = np.linspace(pseudotimes_interp.min(), pseudotimes_interp.max(), 20)
        for i in range(0, max_iter):
            ...
            idx = [
                i for i in range(0, p.shape[0] - 1) if (p[i] != p[i + 1]).any()
            ...
            dist_ind, d_sq = self.project_to_curve(
                X, points=p, pseudotimes=s
            )  # s not used?
        ...
        return (
            spline,
            pseudotimes_uniq.min(),
            pseudotimes_uniq.max(),
            len(pseudotimes_uniq),
            pseudotimes_uniq,
        )

    def dist(self, spline_x, spline_y, start, end, t_min, t_max, num_points):
        t_values = np.linspace(t_min, t_max, num_points)
        x_spline_points = spline_x(t_values)
        y_spline_points = spline_y(t_values)
        start_distances = np.sqrt(
            (x_spline_points - start[0]) ** 2 + (y_spline_points - start[1]) ** 2
        )
        end_distances = np.sqrt(
            (x_spline_points - end[0]) ** 2 + (y_spline_points - end[1]) ** 2
        )
        start_min_index = np.argmin(start_distances)
        end_min_index = np.argmin(end_distances)

        start_nearest_t = t_values[start_min_index]
        end_nearest_t = t_values[end_min_index]
        t_values = np.linspace(start_nearest_t, end_nearest_t, num_points)

        x_spline_points = spline_x(t_values)
        y_spline_points = spline_y(t_values)
        arc_length = np.sum(
            np.sqrt(np.diff(x_spline_points) ** 2 + np.diff(y_spline_points) ** 2)
        )
        return arc_length
