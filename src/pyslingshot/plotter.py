    ...
    for i in range(3):
        for j in range(upper_partitions_half):
            modifier = (
                ...
                - initial_cm[
                    lower_half + j * number_of_shades : lower_half
...
class SlingshotPlotter:
    ...
    def clusters(self, ax, labels=None, s=8, alpha=1.0, color_mode="clusters"):
        fig = plt.gcf()
        sling = self.sling
        if labels is None:
        ...
        ax.scatter(
            ...
            sling.cluster_centres[sling.start_node][1],
            c="red",
        )

        if color_mode == "clusters":
            colors = np.array(
                sns.color_palette("cubehelix", n_colors=sling.num_clusters)
            )
            colors = generate_colormap(sling.num_clusters)

            handles = [
                Patch(color=colors.colors[k], label=labels[k])
                for k in range(sling.num_clusters)
            ]
        ...
        main_scatter = ax.scatter(
            sling.data[:, 0], sling.data[:, 1], c=colors, s=s, alpha=alpha
        )
    ...
    def curves(self, ax, curves, color=None):
    ...
    def network(self, cluster_to_label, figsize=(8, 10)):
        ...
        label_options = dict(ec="k", fc="b", alpha=0.9, boxstyle="round,pad=0.2")

        nx.draw(G, pos, arrows=True, node_size=[len(v) * 100 for v in G.nodes()])
        nx.draw_networkx_labels(
            G, pos, font_size=14, font_color="w", bbox=label_options
        )
